...
static double gaussRandom(double mean, double sigma, double minVal, double maxVal) {
...
do {
    val = dist(rng);
} while ((val < minVal || val > maxVal) && --maxTries > 0);
if (maxTries == 0) {
    fprintf(stderr, "BLEvaluator: gaussRandom() failed to converge within range\n");
    return mean;
}
return val;
}
...
public:
	...
	BLEvaluator() : HepTool::Evaluator() {
		...
		setFunction("gaussRandom", gaussRandom);
	}
