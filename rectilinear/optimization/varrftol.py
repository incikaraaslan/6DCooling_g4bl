def insert_generated_blocks(input_path, output_path, gradients):
    ...
    for i in range(len(gradients)):
        for j,grad in enumerate(gradients[i]):
            # def_block.append(f"# Cavity {j} in Cavity Block {i} with gradient {grad} MV/m\n")
            def_block.append(f"pillbox cav{i}{j} innerLength=$rf_length irisRadius=$pipe_radius frequency=$rf_fre maxGradient={grad} phaseAcc=$rf_ph winMat=Be cavityMaterial=$cavity_gas win1Thick=$rf_window_length win2Thick=$rf_window_length wallThick=$wallthick collarThick=0 kill=1")
            def_block.append("\n")
    def_block.append("\n")
...
insert_generated_blocks("incis_cleaned_cooling_stage1_variablerfs.g4bl", "incis_cleaned_cooling_stage1_TEST.g4bl", gradients)"""