...
if __name__ == "__main__":
    ...
    plt.fill_between(zvals[0], np.array(eperps_avg) - np.array(eperps_sem), np.array(eperps_avg) + np.array(eperps_sem), color='gray', alpha=0.3, label=f'±{stdev} SEM')
    plt.xlabel("z [m]")
    plt.ylabel(r"$\epsilon_T$ [mm]")
    plt.title("Transverse Emittance v. Beam Axis (z)")
    plt.grid(True)
    plt.legend()
    plt.savefig("./varrfgradtol_eperp"+"_sample-size_" + str(args.sample_size) +"_"+str(args.number_of_particles) + "_"+str(args.stdev)+"SEM.png")
    plt.close()
    ...
    plt.fill_between(zvals[0], np.array(elongs_avg) - np.array(elongs_sem), np.array(elongs_avg) + np.array(elongs_sem), color='gray', alpha=0.3, label=f'±{stdev} SEM')
    plt.xlabel("z [m]")
    plt.ylabel(r"$\epsilon_L$ [mm]")
    plt.title("Longitudinal Emittance v. Beam Axis (z)")
    plt.grid(True)
    plt.legend()
    plt.savefig("./varrfgradtol_elong"+"_sample-size_" + str(args.sample_size) +"_"+str(args.number_of_particles)+"_"+str(args.stdev)+"SEM.png")
    plt.close()
    ...
    plt.fill_between(zvals[0], np.array(transs_avg) - np.array(transs_sem), np.array(transs_avg) + np.array(transs_sem), color='gray', alpha=0.3, label=f'±{stdev} SEM')
    plt.xlabel("z [m]")
    plt.ylabel("Transmission [%]")
    plt.title("Transmission v. Beam Axis (z)")
    plt.grid(True)
    plt.legend()
    plt.savefig("./varrfgradtol_trans"+"_sample-size_" + str(args.sample_size) +"_"+str(args.number_of_particles) +"_"+str(args.stdev)+"SEM.png")
    plt.close()
