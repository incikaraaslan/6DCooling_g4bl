...
def insert_generated_blocks(input_path, output_path, gradients, phases):
    ...
    prad = re.search(r'param\s+pipe_radius\s*=\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)', content)
    ...
    for i in range(len(gradients)):
        for j, (grad,ph) in enumerate(zip(gradients[i], phases[i])):
            ...
            if int(prad.group(1)) < 50:
                def_block.append(f"pillbox cav{i}{j} innerLength=$rf_length irisRadius=$pipe_radius frequency=$rf_fre maxGradient={grad} phaseAcc={ph} winMat=Be cavityMaterial=$cavity_gas win1Thick=$rf_window_length win1OuterRadius=$win1out_radius win2Thick=$rf_window_length wallThick=$wallthick collarThick=0 kill=1")
                def_block.append("\n")
            else:
