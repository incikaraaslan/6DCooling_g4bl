...
def singleStage(input_file, mean, stdev, meanp, stdevp, no_cavities, ncells, number_of_particles, stage_no, sample_no): #SINGLE SAMPLE!
    ...
    if os.path.exists(f"./for009_werr_{stage_no}_{sample_no}.txt"):
        ...
        if int(stage_no) < 5:
            ...
            data_final = np.loadtxt("./ecalc9f.dat",skiprows=13, usecols=cols)
        else:
        ...
        e6ds = data_final[:,4]*(1000**3) # mm to m conversion
        transs= data_final[:,5]/data_final[0,5]*100 # Percentage Conversion
...
def singleStageNoErr(noerrfile, number_of_particles, stage_no):
    ...
    if os.path.exists(f"./for009_noerr_{stage_no}.txt"):
        ...
        e6ds_noerr = data_final[:,4]*(1000**3) # mm to m conversion
        transs_noerr = data_final[:,5]/data_final[0,5]*100 # Percentage Conversion
