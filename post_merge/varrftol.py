...
def insert_generated_blocks(input_path, output_path, gradients, phases):
    ...
    if no_cavs == 6:
        ...
        for i in range(len(gradients)): # Run n cell times each times
            place_block.append(f"\nplace cav{i}0 z={i}*$cell_length+$cell_length/2-(2*$rf_length+2*2*$wallthick+$wallthick+$rf_length/2) rename=rf_a_{i} \nplace cav{i}1 z={i}*$cell_length+$cell_length/2-($rf_length+2*$wallthick+$wallthick+$rf_length/2) rename=rf_b_{i}\nplace cav{i}2 z={i}*$cell_length+$cell_length/2-($wallthick+$rf_length/2) rename=rf_c_{i} \nplace cav{i}3 z={i}*$cell_length+$cell_length/2+($wallthick+$rf_length/2) rename=rf_d_{i} \nplace cav{i}4 z={i}*$cell_length+$cell_length/2+($rf_length+2*$wallthick+$wallthick+$rf_length/2) rename=rf_e_{i} \nplace cav{i}5 z={i}*$cell_length+$cell_length/2+(2*$rf_length+2*2*$wallthick+$wallthick+$rf_length/2) rename=rf_f_{i} \n")
        def_block.append("\n")
        ...
        place_block = []
        for i in range(len(gradients)): # Run n cell times each times
            place_block.append(f"\nplace cav{i}0 z={i}*$cell_length+$cell_length/2-(4*$wallthick+2*$rf_length) rename=rf_b_{i} \nplace cav{i}1 z={i}*$cell_length+$cell_length/2-(2*$wallthick+$rf_length) rename=rf_c_{i}\nplace cav{i}2 z={i}*$cell_length+$cell_length/2 rename=rf_d_{i} \nplace cav{i}3 z={i}*$cell_length+$cell_length/2+(2*$wallthick+$rf_length) rename=rf_e_{i} \nplace cav{i}4 z={i}*$cell_length+$cell_length/2+(4*$wallthick+2*$rf_length) rename=rf_f_{i}\n")
        def_block.append("\n")
        # Rebuild file with inserted content
        new_lines = (
            lines[:def_start_idx]
        ...
        place_block = []
        for i in range(len(gradients)): # Run n cell times each times
            place_block.append(f"\nplace cav{i}0 z={i}*$cell_length+$cell_length/2-($rf_length+2*$wallthick+$wallthick+$rf_length/2) rename=rf_b_{i} \nplace cav{i}1 z={i}*$cell_length+$cell_length/2-($wallthick+$rf_length/2) rename=rf_c_{i} \nplace cav{i}2 z={i}*$cell_length+$cell_length/2+($wallthick+$rf_length/2) rename=rf_d_{i} \nplace cav{i}3 z={i}*$cell_length+$cell_length/2+($rf_length+2*$wallthick+$wallthick+$rf_length/2) rename=rf_e_{i}\n")
        def_block.append("\n")
        # Rebuild file with inserted content
        new_lines = (
            lines[:def_start_idx]
        ...
        place_block = []
        for i in range(len(gradients)): # Run n cell times each times
            place_block.append(f"\nplace cav{i}0 z={i}*$cell_length+$cell_length/2-(2*$wallthick+$rf_length) rename=rf_c_{i} \nplace cav{i}1 z={i}*$cell_length+$cell_length/2 rename=rf_d_{i} \nplace cav{i}2 z={i}*$cell_length+$cell_length/2+(2*$wallthick+$rf_length) rename=rf_e_{i}\n")
        def_block.append("\n")
        # Rebuild file with inserted content
        new_lines = (
            lines[:def_start_idx]
